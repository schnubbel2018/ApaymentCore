...
*Setup instructions for a gitian build of Apollon using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Apollon
...
- [Building Apollon](#building-apollon)
...
Clone the git repositories for apollon and gitian and then checkout the apollon version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/apollon-crypto/apollon.git
cd apollon
...
Follow the instructions in [doc/release-process.md](release-process.md) in the apollon repository
...
To build Apollon (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the apollon repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/apollon/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/apollon-crypto/apollon
...
For example:
```bash
URL=https://github.com/crowning-/apollon.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit apollon=${COMMIT} --url apollon=${URL} ../apollon/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit apollon=${COMMIT} --url apollon=${URL} ../apollon/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit apollon=${COMMIT} --url apollon=${URL} ../apollon/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/apollon-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/apollon-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/apollon-build.assert
```
...
[apollon/gitian.sigs](https://github.com/apollon-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
