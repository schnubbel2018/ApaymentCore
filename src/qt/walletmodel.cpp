...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered apollon address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 XAP.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal apollon:URI (apollon:XyZ...?message=example)
            {
...
static void NotifyzXAPReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzXAPReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzXAPReset.connect(boost::bind(NotifyzXAPReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzXAPReset.disconnect(boost::bind(NotifyzXAPReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zxapTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
