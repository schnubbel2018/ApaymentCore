...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::XAP)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sXAPPercentage, QString& szXAPPercentage)
{
    int nPrecision = 2;
    ...
    szXAPPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sXAPPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount xapAvailableBalance = balance - immatureBalance;
    ...
    bool showWatchOnlyXAPPending = watchUnconfBalance != 0;
    bool showXAPLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyXAPLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showXAPAvailable || showWatchOnlyXAPAvailable);
    ui->labelBalanceText->setVisible(showXAPAvailable || showWatchOnlyXAPAvailable);
    ui->labelWatchAvailable->setVisible(showXAPAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showXAPPending || showWatchOnlyXAPPending);
    ui->labelPendingText->setVisible(showXAPPending || showWatchOnlyXAPPending);
    ui->labelWatchPending->setVisible(showXAPPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showXAPLocked || showWatchOnlyXAPLocked);
    ui->labelLockedBalanceText->setVisible(showXAPLocked || showWatchOnlyXAPLocked);
    ui->labelWatchLocked->setVisible(showXAPLocked && showWatchOnly);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("XAP")
    updateDisplayUnit();
}
