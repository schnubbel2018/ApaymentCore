...
public:
    ...
    bool WriteZXAPSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZXAPSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZXAPSeed_deprecated(uint256& seed);
    bool EraseZXAPSeed();
    bool EraseZXAPSeed_deprecated();

    bool WriteZXAPCount(const uint32_t& nCount);
    bool ReadZXAPCount(uint32_t& nCount);
