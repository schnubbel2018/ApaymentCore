...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-backupzxap=<n>", strprintf(_("Enable automatic wallet backups triggered after each zXAP minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zxapbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zXAP backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizeapollonamount=<n>     " + strprintf(_("Keep N XAP anonymized (default: %u)"), 0) + "\n";
...
//    }
//
//    nAnonymizeXapxAmount = GetArg("-anonymizeapollonamount", 0);
//    if (nAnonymizeXapxAmount > 999999) nAnonymizeXapxAmount = 999999;
//    if (nAnonymizeXapxAmount < 2) nAnonymizeXapxAmount = 2;
